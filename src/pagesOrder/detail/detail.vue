...
onReady(() => {
  ...
  pageInstance.animate('.navbar .title', [{ color: 'transparent' }, { color: '#f8f8f8' }], 1000, {
    scrollSource: '#scroller',
    TimeRanges: 1000,
    startScrollOffset: 0,
    endScrollOffset: 50,
...
const getOrderData = async () => {
  const res: any = await getMemberOrder(query.id)
...
const getMemberOrderLogisticsByIdData = async () => {
  const res: any = await getMemberOrderLogisticsByIdAPI(query.id)
    ...
    <template v-if="order?.orderState === OrderState.DaiFuKuan">
      ...
      <view class="overview" :style="{ paddingTop: safeAreaInsets!.top + 20 + 'px' }">
        ...
        <template v-if="true">
          ...
          <view class="tips">
            <text class="money">应付金额: ¥ {{ order.payMonet }}</text>
      ...
      <view class="shipment">
        <!-- 订单物流信息 -->
        <view v-for="item in logisticsList" :key="item" class="item">
          <view class="message">
            {{ item.text }}
          </view>
